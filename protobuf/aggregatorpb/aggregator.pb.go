// Code generated by protoc-gen-gogo. DO NOT EDIT.
// source: protobuf/aggregatorpb/aggregator.proto

/*
	Package aggregatorpb is a generated protocol buffer package.

	It is generated from these files:
		protobuf/aggregatorpb/aggregator.proto

	It has these top-level messages:
		HyperParams
		SchedulingInfo
		Work
		ModelID
		GetWorkRequest
		FindWorkRequest
		FindWorkResponse
		ModelWeightChunk
		GetWorkResponse
		ReportWorkRequest
		ReportWorkResponse
		ProdModelRequest
		ProdModelResponse
		ModelTypeAllocation
		NotifyRequest
		NotifyResponse
		CancelModelTrainingRequest
		CancelModelTrainingResponse
		ModelURLRequest
		ModelURLResponse
		Error
		ReportErrorRequest
		ReportErrorResponse
*/
package aggregatorpb

import proto "github.com/gogo/protobuf/proto"
import fmt "fmt"
import math "math"
import _ "github.com/gogo/protobuf/gogoproto"
import _ "github.com/golang/protobuf/ptypes/timestamp"

import time "time"

import strconv "strconv"

import bytes "bytes"

import strings "strings"
import reflect "reflect"

import context "golang.org/x/net/context"
import grpc "google.golang.org/grpc"

import binary "encoding/binary"
import types "github.com/gogo/protobuf/types"

import io "io"

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf
var _ = time.Kitchen

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.GoGoProtoPackageIsVersion2 // please upgrade the proto package

type TrainingStatus int32

const (
	SCHEDULED TrainingStatus = 0
	TRAINING  TrainingStatus = 1
	SUCCEEDED TrainingStatus = 2
	FAILED    TrainingStatus = 3
	CANCELED  TrainingStatus = 4
)

var TrainingStatus_name = map[int32]string{
	0: "SCHEDULED",
	1: "TRAINING",
	2: "SUCCEEDED",
	3: "FAILED",
	4: "CANCELED",
}
var TrainingStatus_value = map[string]int32{
	"SCHEDULED": 0,
	"TRAINING":  1,
	"SUCCEEDED": 2,
	"FAILED":    3,
	"CANCELED":  4,
}

func (TrainingStatus) EnumDescriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{0} }

type Repeat int32

const (
	NEVER   Repeat = 0
	HOURLY  Repeat = 1
	DAILY   Repeat = 2
	WEEKLY  Repeat = 3
	MONTHLY Repeat = 4
)

var Repeat_name = map[int32]string{
	0: "NEVER",
	1: "HOURLY",
	2: "DAILY",
	3: "WEEKLY",
	4: "MONTHLY",
}
var Repeat_value = map[string]int32{
	"NEVER":   0,
	"HOURLY":  1,
	"DAILY":   2,
	"WEEKLY":  3,
	"MONTHLY": 4,
}

func (Repeat) EnumDescriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{1} }

type ErrorType int32

const (
	ERROR_UNKNOWN   ErrorType = 0
	ERROR_TRAINING  ErrorType = 1
	ERROR_INFERENCE ErrorType = 2
)

var ErrorType_name = map[int32]string{
	0: "ERROR_UNKNOWN",
	1: "ERROR_TRAINING",
	2: "ERROR_INFERENCE",
}
var ErrorType_value = map[string]int32{
	"ERROR_UNKNOWN":   0,
	"ERROR_TRAINING":  1,
	"ERROR_INFERENCE": 2,
}

func (ErrorType) EnumDescriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{2} }

type HyperParams struct {
	// double proportion_clients = 1;
	BatchSize        int64   `protobuf:"varint,2,opt,name=batch_size,json=batchSize,proto3" json:"batch_size,omitempty"`
	NumRounds        int64   `protobuf:"varint,3,opt,name=num_rounds,json=numRounds,proto3" json:"num_rounds,omitempty"`
	LearningRate     float64 `protobuf:"fixed64,4,opt,name=learning_rate,json=learningRate,proto3" json:"learning_rate,omitempty"`
	NumLocalRounds   int64   `protobuf:"varint,5,opt,name=num_local_rounds,json=numLocalRounds,proto3" json:"num_local_rounds,omitempty"`
	QuantizedUpdates bool    `protobuf:"varint,6,opt,name=quantized_updates,json=quantizedUpdates,proto3" json:"quantized_updates,omitempty"`
	NumClients       int64   `protobuf:"varint,7,opt,name=num_clients,json=numClients,proto3" json:"num_clients,omitempty"`
}

func (m *HyperParams) Reset()                    { *m = HyperParams{} }
func (*HyperParams) ProtoMessage()               {}
func (*HyperParams) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{0} }

func (m *HyperParams) GetBatchSize() int64 {
	if m != nil {
		return m.BatchSize
	}
	return 0
}

func (m *HyperParams) GetNumRounds() int64 {
	if m != nil {
		return m.NumRounds
	}
	return 0
}

func (m *HyperParams) GetLearningRate() float64 {
	if m != nil {
		return m.LearningRate
	}
	return 0
}

func (m *HyperParams) GetNumLocalRounds() int64 {
	if m != nil {
		return m.NumLocalRounds
	}
	return 0
}

func (m *HyperParams) GetQuantizedUpdates() bool {
	if m != nil {
		return m.QuantizedUpdates
	}
	return false
}

func (m *HyperParams) GetNumClients() int64 {
	if m != nil {
		return m.NumClients
	}
	return 0
}

type SchedulingInfo struct {
	Scheduled *time.Time `protobuf:"bytes,1,opt,name=scheduled,stdtime" json:"scheduled,omitempty"`
	Repeat    Repeat     `protobuf:"varint,2,opt,name=repeat,proto3,enum=aggregatorpb.Repeat" json:"repeat,omitempty"`
}

func (m *SchedulingInfo) Reset()                    { *m = SchedulingInfo{} }
func (*SchedulingInfo) ProtoMessage()               {}
func (*SchedulingInfo) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{1} }

func (m *SchedulingInfo) GetScheduled() *time.Time {
	if m != nil {
		return m.Scheduled
	}
	return nil
}

func (m *SchedulingInfo) GetRepeat() Repeat {
	if m != nil {
		return m.Repeat
	}
	return NEVER
}

type Work struct {
	Id          ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
	NumExamples int64   `protobuf:"varint,2,opt,name=num_examples,json=numExamples,proto3" json:"num_examples,omitempty"`
	NumClients  int64   `protobuf:"varint,3,opt,name=num_clients,json=numClients,proto3" json:"num_clients,omitempty"`
	Epoch       int64   `protobuf:"varint,4,opt,name=epoch,proto3" json:"epoch,omitempty"`
	// TODO(d4l3k): consider sending a link to GCS instead of the raw model.
	HyperParams HyperParams `protobuf:"bytes,7,opt,name=hyper_params,json=hyperParams" json:"hyper_params"`
	// time_taken is amount of time in seconds it took to process this work.
	TimeTaken float64 `protobuf:"fixed64,8,opt,name=time_taken,json=timeTaken,proto3" json:"time_taken,omitempty"`
	// started is when the training job was started.
	Started time.Time `protobuf:"bytes,11,opt,name=started,stdtime" json:"started"`
	// metrics are the values produced by the model defined metrics.
	Metrics []float64 `protobuf:"fixed64,9,rep,packed,name=metrics" json:"metrics,omitempty"`
}

func (m *Work) Reset()                    { *m = Work{} }
func (*Work) ProtoMessage()               {}
func (*Work) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{2} }

func (m *Work) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

func (m *Work) GetNumExamples() int64 {
	if m != nil {
		return m.NumExamples
	}
	return 0
}

func (m *Work) GetNumClients() int64 {
	if m != nil {
		return m.NumClients
	}
	return 0
}

func (m *Work) GetEpoch() int64 {
	if m != nil {
		return m.Epoch
	}
	return 0
}

func (m *Work) GetHyperParams() HyperParams {
	if m != nil {
		return m.HyperParams
	}
	return HyperParams{}
}

func (m *Work) GetTimeTaken() float64 {
	if m != nil {
		return m.TimeTaken
	}
	return 0
}

func (m *Work) GetStarted() time.Time {
	if m != nil {
		return m.Started
	}
	return time.Time{}
}

func (m *Work) GetMetrics() []float64 {
	if m != nil {
		return m.Metrics
	}
	return nil
}

type ModelID struct {
	Domain    string `protobuf:"bytes,1,opt,name=domain,proto3" json:"domain,omitempty"`
	ModelType string `protobuf:"bytes,2,opt,name=model_type,json=modelType,proto3" json:"model_type,omitempty"`
	Id        uint64 `protobuf:"varint,3,opt,name=id,proto3" json:"id,omitempty"`
}

func (m *ModelID) Reset()                    { *m = ModelID{} }
func (*ModelID) ProtoMessage()               {}
func (*ModelID) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{3} }

func (m *ModelID) GetDomain() string {
	if m != nil {
		return m.Domain
	}
	return ""
}

func (m *ModelID) GetModelType() string {
	if m != nil {
		return m.ModelType
	}
	return ""
}

func (m *ModelID) GetId() uint64 {
	if m != nil {
		return m.Id
	}
	return 0
}

type GetWorkRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *GetWorkRequest) Reset()                    { *m = GetWorkRequest{} }
func (*GetWorkRequest) ProtoMessage()               {}
func (*GetWorkRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{4} }

func (m *GetWorkRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type FindWorkRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *FindWorkRequest) Reset()                    { *m = FindWorkRequest{} }
func (*FindWorkRequest) ProtoMessage()               {}
func (*FindWorkRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{5} }

func (m *FindWorkRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type FindWorkResponse struct {
	AggregatorAddr string `protobuf:"bytes,1,opt,name=aggregator_addr,json=aggregatorAddr,proto3" json:"aggregator_addr,omitempty"`
}

func (m *FindWorkResponse) Reset()                    { *m = FindWorkResponse{} }
func (*FindWorkResponse) ProtoMessage()               {}
func (*FindWorkResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{6} }

func (m *FindWorkResponse) GetAggregatorAddr() string {
	if m != nil {
		return m.AggregatorAddr
	}
	return ""
}

type ModelWeightChunk struct {
	Body []byte `protobuf:"bytes,1,opt,name=body,proto3" json:"body,omitempty"`
	More bool   `protobuf:"varint,2,opt,name=more,proto3" json:"more,omitempty"`
	// id is the chunk number.
	Id uint64 `protobuf:"varint,3,opt,name=id,proto3" json:"id,omitempty"`
}

func (m *ModelWeightChunk) Reset()                    { *m = ModelWeightChunk{} }
func (*ModelWeightChunk) ProtoMessage()               {}
func (*ModelWeightChunk) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{7} }

func (m *ModelWeightChunk) GetBody() []byte {
	if m != nil {
		return m.Body
	}
	return nil
}

func (m *ModelWeightChunk) GetMore() bool {
	if m != nil {
		return m.More
	}
	return false
}

func (m *ModelWeightChunk) GetId() uint64 {
	if m != nil {
		return m.Id
	}
	return 0
}

type GetWorkResponse struct {
	// Types that are valid to be assigned to Type:
	//	*GetWorkResponse_Work
	//	*GetWorkResponse_Weights
	Type isGetWorkResponse_Type `protobuf_oneof:"type"`
}

func (m *GetWorkResponse) Reset()                    { *m = GetWorkResponse{} }
func (*GetWorkResponse) ProtoMessage()               {}
func (*GetWorkResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{8} }

type isGetWorkResponse_Type interface {
	isGetWorkResponse_Type()
	Equal(interface{}) bool
	MarshalTo([]byte) (int, error)
	Size() int
}

type GetWorkResponse_Work struct {
	Work *Work `protobuf:"bytes,1,opt,name=work,oneof"`
}
type GetWorkResponse_Weights struct {
	Weights *ModelWeightChunk `protobuf:"bytes,2,opt,name=weights,oneof"`
}

func (*GetWorkResponse_Work) isGetWorkResponse_Type()    {}
func (*GetWorkResponse_Weights) isGetWorkResponse_Type() {}

func (m *GetWorkResponse) GetType() isGetWorkResponse_Type {
	if m != nil {
		return m.Type
	}
	return nil
}

func (m *GetWorkResponse) GetWork() *Work {
	if x, ok := m.GetType().(*GetWorkResponse_Work); ok {
		return x.Work
	}
	return nil
}

func (m *GetWorkResponse) GetWeights() *ModelWeightChunk {
	if x, ok := m.GetType().(*GetWorkResponse_Weights); ok {
		return x.Weights
	}
	return nil
}

// XXX_OneofFuncs is for the internal use of the proto package.
func (*GetWorkResponse) XXX_OneofFuncs() (func(msg proto.Message, b *proto.Buffer) error, func(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error), func(msg proto.Message) (n int), []interface{}) {
	return _GetWorkResponse_OneofMarshaler, _GetWorkResponse_OneofUnmarshaler, _GetWorkResponse_OneofSizer, []interface{}{
		(*GetWorkResponse_Work)(nil),
		(*GetWorkResponse_Weights)(nil),
	}
}

func _GetWorkResponse_OneofMarshaler(msg proto.Message, b *proto.Buffer) error {
	m := msg.(*GetWorkResponse)
	// type
	switch x := m.Type.(type) {
	case *GetWorkResponse_Work:
		_ = b.EncodeVarint(1<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.Work); err != nil {
			return err
		}
	case *GetWorkResponse_Weights:
		_ = b.EncodeVarint(2<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.Weights); err != nil {
			return err
		}
	case nil:
	default:
		return fmt.Errorf("GetWorkResponse.Type has unexpected type %T", x)
	}
	return nil
}

func _GetWorkResponse_OneofUnmarshaler(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error) {
	m := msg.(*GetWorkResponse)
	switch tag {
	case 1: // type.work
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(Work)
		err := b.DecodeMessage(msg)
		m.Type = &GetWorkResponse_Work{msg}
		return true, err
	case 2: // type.weights
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(ModelWeightChunk)
		err := b.DecodeMessage(msg)
		m.Type = &GetWorkResponse_Weights{msg}
		return true, err
	default:
		return false, nil
	}
}

func _GetWorkResponse_OneofSizer(msg proto.Message) (n int) {
	m := msg.(*GetWorkResponse)
	// type
	switch x := m.Type.(type) {
	case *GetWorkResponse_Work:
		s := proto.Size(x.Work)
		n += proto.SizeVarint(1<<3 | proto.WireBytes)
		n += proto.SizeVarint(uint64(s))
		n += s
	case *GetWorkResponse_Weights:
		s := proto.Size(x.Weights)
		n += proto.SizeVarint(2<<3 | proto.WireBytes)
		n += proto.SizeVarint(uint64(s))
		n += s
	case nil:
	default:
		panic(fmt.Sprintf("proto: unexpected type %T in oneof", x))
	}
	return n
}

type ReportWorkRequest struct {
	// Types that are valid to be assigned to Type:
	//	*ReportWorkRequest_Work
	//	*ReportWorkRequest_Weights
	Type isReportWorkRequest_Type `protobuf_oneof:"type"`
}

func (m *ReportWorkRequest) Reset()                    { *m = ReportWorkRequest{} }
func (*ReportWorkRequest) ProtoMessage()               {}
func (*ReportWorkRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{9} }

type isReportWorkRequest_Type interface {
	isReportWorkRequest_Type()
	Equal(interface{}) bool
	MarshalTo([]byte) (int, error)
	Size() int
}

type ReportWorkRequest_Work struct {
	Work *Work `protobuf:"bytes,1,opt,name=work,oneof"`
}
type ReportWorkRequest_Weights struct {
	Weights *ModelWeightChunk `protobuf:"bytes,2,opt,name=weights,oneof"`
}

func (*ReportWorkRequest_Work) isReportWorkRequest_Type()    {}
func (*ReportWorkRequest_Weights) isReportWorkRequest_Type() {}

func (m *ReportWorkRequest) GetType() isReportWorkRequest_Type {
	if m != nil {
		return m.Type
	}
	return nil
}

func (m *ReportWorkRequest) GetWork() *Work {
	if x, ok := m.GetType().(*ReportWorkRequest_Work); ok {
		return x.Work
	}
	return nil
}

func (m *ReportWorkRequest) GetWeights() *ModelWeightChunk {
	if x, ok := m.GetType().(*ReportWorkRequest_Weights); ok {
		return x.Weights
	}
	return nil
}

// XXX_OneofFuncs is for the internal use of the proto package.
func (*ReportWorkRequest) XXX_OneofFuncs() (func(msg proto.Message, b *proto.Buffer) error, func(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error), func(msg proto.Message) (n int), []interface{}) {
	return _ReportWorkRequest_OneofMarshaler, _ReportWorkRequest_OneofUnmarshaler, _ReportWorkRequest_OneofSizer, []interface{}{
		(*ReportWorkRequest_Work)(nil),
		(*ReportWorkRequest_Weights)(nil),
	}
}

func _ReportWorkRequest_OneofMarshaler(msg proto.Message, b *proto.Buffer) error {
	m := msg.(*ReportWorkRequest)
	// type
	switch x := m.Type.(type) {
	case *ReportWorkRequest_Work:
		_ = b.EncodeVarint(1<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.Work); err != nil {
			return err
		}
	case *ReportWorkRequest_Weights:
		_ = b.EncodeVarint(2<<3 | proto.WireBytes)
		if err := b.EncodeMessage(x.Weights); err != nil {
			return err
		}
	case nil:
	default:
		return fmt.Errorf("ReportWorkRequest.Type has unexpected type %T", x)
	}
	return nil
}

func _ReportWorkRequest_OneofUnmarshaler(msg proto.Message, tag, wire int, b *proto.Buffer) (bool, error) {
	m := msg.(*ReportWorkRequest)
	switch tag {
	case 1: // type.work
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(Work)
		err := b.DecodeMessage(msg)
		m.Type = &ReportWorkRequest_Work{msg}
		return true, err
	case 2: // type.weights
		if wire != proto.WireBytes {
			return true, proto.ErrInternalBadWireType
		}
		msg := new(ModelWeightChunk)
		err := b.DecodeMessage(msg)
		m.Type = &ReportWorkRequest_Weights{msg}
		return true, err
	default:
		return false, nil
	}
}

func _ReportWorkRequest_OneofSizer(msg proto.Message) (n int) {
	m := msg.(*ReportWorkRequest)
	// type
	switch x := m.Type.(type) {
	case *ReportWorkRequest_Work:
		s := proto.Size(x.Work)
		n += proto.SizeVarint(1<<3 | proto.WireBytes)
		n += proto.SizeVarint(uint64(s))
		n += s
	case *ReportWorkRequest_Weights:
		s := proto.Size(x.Weights)
		n += proto.SizeVarint(2<<3 | proto.WireBytes)
		n += proto.SizeVarint(uint64(s))
		n += s
	case nil:
	default:
		panic(fmt.Sprintf("proto: unexpected type %T in oneof", x))
	}
	return n
}

type ReportWorkResponse struct {
}

func (m *ReportWorkResponse) Reset()                    { *m = ReportWorkResponse{} }
func (*ReportWorkResponse) ProtoMessage()               {}
func (*ReportWorkResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{10} }

type ProdModelRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *ProdModelRequest) Reset()                    { *m = ProdModelRequest{} }
func (*ProdModelRequest) ProtoMessage()               {}
func (*ProdModelRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{11} }

func (m *ProdModelRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type ProdModelResponse struct {
	Id ModelID `protobuf:"bytes,3,opt,name=id" json:"id"`
}

func (m *ProdModelResponse) Reset()                    { *m = ProdModelResponse{} }
func (*ProdModelResponse) ProtoMessage()               {}
func (*ProdModelResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{12} }

func (m *ProdModelResponse) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type ModelTypeAllocation struct {
	ModelType ModelID `protobuf:"bytes,1,opt,name=model_type,json=modelType" json:"model_type"`
	Addr      string  `protobuf:"bytes,2,opt,name=addr,proto3" json:"addr,omitempty"`
}

func (m *ModelTypeAllocation) Reset()                    { *m = ModelTypeAllocation{} }
func (*ModelTypeAllocation) ProtoMessage()               {}
func (*ModelTypeAllocation) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{13} }

func (m *ModelTypeAllocation) GetModelType() ModelID {
	if m != nil {
		return m.ModelType
	}
	return ModelID{}
}

func (m *ModelTypeAllocation) GetAddr() string {
	if m != nil {
		return m.Addr
	}
	return ""
}

type NotifyRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *NotifyRequest) Reset()                    { *m = NotifyRequest{} }
func (*NotifyRequest) ProtoMessage()               {}
func (*NotifyRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{14} }

func (m *NotifyRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type NotifyResponse struct {
}

func (m *NotifyResponse) Reset()                    { *m = NotifyResponse{} }
func (*NotifyResponse) ProtoMessage()               {}
func (*NotifyResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{15} }

type CancelModelTrainingRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *CancelModelTrainingRequest) Reset()      { *m = CancelModelTrainingRequest{} }
func (*CancelModelTrainingRequest) ProtoMessage() {}
func (*CancelModelTrainingRequest) Descriptor() ([]byte, []int) {
	return fileDescriptorAggregator, []int{16}
}

func (m *CancelModelTrainingRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type CancelModelTrainingResponse struct {
}

func (m *CancelModelTrainingResponse) Reset()      { *m = CancelModelTrainingResponse{} }
func (*CancelModelTrainingResponse) ProtoMessage() {}
func (*CancelModelTrainingResponse) Descriptor() ([]byte, []int) {
	return fileDescriptorAggregator, []int{17}
}

type ModelURLRequest struct {
	Id ModelID `protobuf:"bytes,1,opt,name=id" json:"id"`
}

func (m *ModelURLRequest) Reset()                    { *m = ModelURLRequest{} }
func (*ModelURLRequest) ProtoMessage()               {}
func (*ModelURLRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{18} }

func (m *ModelURLRequest) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

type ModelURLResponse struct {
	Url string `protobuf:"bytes,1,opt,name=url,proto3" json:"url,omitempty"`
}

func (m *ModelURLResponse) Reset()                    { *m = ModelURLResponse{} }
func (*ModelURLResponse) ProtoMessage()               {}
func (*ModelURLResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{19} }

func (m *ModelURLResponse) GetUrl() string {
	if m != nil {
		return m.Url
	}
	return ""
}

type Error struct {
	Id         ModelID   `protobuf:"bytes,1,opt,name=id" json:"id"`
	Type       ErrorType `protobuf:"varint,2,opt,name=type,proto3,enum=aggregatorpb.ErrorType" json:"type,omitempty"`
	Error      string    `protobuf:"bytes,3,opt,name=error,proto3" json:"error,omitempty"`
	Stacktrace string    `protobuf:"bytes,4,opt,name=stacktrace,proto3" json:"stacktrace,omitempty"`
}

func (m *Error) Reset()                    { *m = Error{} }
func (*Error) ProtoMessage()               {}
func (*Error) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{20} }

func (m *Error) GetId() ModelID {
	if m != nil {
		return m.Id
	}
	return ModelID{}
}

func (m *Error) GetType() ErrorType {
	if m != nil {
		return m.Type
	}
	return ERROR_UNKNOWN
}

func (m *Error) GetError() string {
	if m != nil {
		return m.Error
	}
	return ""
}

func (m *Error) GetStacktrace() string {
	if m != nil {
		return m.Stacktrace
	}
	return ""
}

type ReportErrorRequest struct {
	Errors []Error `protobuf:"bytes,1,rep,name=errors" json:"errors"`
}

func (m *ReportErrorRequest) Reset()                    { *m = ReportErrorRequest{} }
func (*ReportErrorRequest) ProtoMessage()               {}
func (*ReportErrorRequest) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{21} }

func (m *ReportErrorRequest) GetErrors() []Error {
	if m != nil {
		return m.Errors
	}
	return nil
}

type ReportErrorResponse struct {
}

func (m *ReportErrorResponse) Reset()                    { *m = ReportErrorResponse{} }
func (*ReportErrorResponse) ProtoMessage()               {}
func (*ReportErrorResponse) Descriptor() ([]byte, []int) { return fileDescriptorAggregator, []int{22} }

func init() {
	proto.RegisterType((*HyperParams)(nil), "aggregatorpb.HyperParams")
	proto.RegisterType((*SchedulingInfo)(nil), "aggregatorpb.SchedulingInfo")
	proto.RegisterType((*Work)(nil), "aggregatorpb.Work")
	proto.RegisterType((*ModelID)(nil), "aggregatorpb.ModelID")
	proto.RegisterType((*GetWorkRequest)(nil), "aggregatorpb.GetWorkRequest")
	proto.RegisterType((*FindWorkRequest)(nil), "aggregatorpb.FindWorkRequest")
	proto.RegisterType((*FindWorkResponse)(nil), "aggregatorpb.FindWorkResponse")
	proto.RegisterType((*ModelWeightChunk)(nil), "aggregatorpb.ModelWeightChunk")
	proto.RegisterType((*GetWorkResponse)(nil), "aggregatorpb.GetWorkResponse")
	proto.RegisterType((*ReportWorkRequest)(nil), "aggregatorpb.ReportWorkRequest")
	proto.RegisterType((*ReportWorkResponse)(nil), "aggregatorpb.ReportWorkResponse")
	proto.RegisterType((*ProdModelRequest)(nil), "aggregatorpb.ProdModelRequest")
	proto.RegisterType((*ProdModelResponse)(nil), "aggregatorpb.ProdModelResponse")
	proto.RegisterType((*ModelTypeAllocation)(nil), "aggregatorpb.ModelTypeAllocation")
	proto.RegisterType((*NotifyRequest)(nil), "aggregatorpb.NotifyRequest")
	proto.RegisterType((*NotifyResponse)(nil), "aggregatorpb.NotifyResponse")
	proto.RegisterType((*CancelModelTrainingRequest)(nil), "aggregatorpb.CancelModelTrainingRequest")
	proto.RegisterType((*CancelModelTrainingResponse)(nil), "aggregatorpb.CancelModelTrainingResponse")
	proto.RegisterType((*ModelURLRequest)(nil), "aggregatorpb.ModelURLRequest")
	proto.RegisterType((*ModelURLResponse)(nil), "aggregatorpb.ModelURLResponse")
	proto.RegisterType((*Error)(nil), "aggregatorpb.Error")
	proto.RegisterType((*ReportErrorRequest)(nil), "aggregatorpb.ReportErrorRequest")
	proto.RegisterType((*ReportErrorResponse)(nil), "aggregatorpb.ReportErrorResponse")
	proto.RegisterEnum("aggregatorpb.TrainingStatus", TrainingStatus_name, TrainingStatus_value)
	proto.RegisterEnum("aggregatorpb.Repeat", Repeat_name, Repeat_value)
	proto.RegisterEnum("aggregatorpb.ErrorType", ErrorType_name, ErrorType_value)
}
func (x TrainingStatus) String() string {
	s, ok := TrainingStatus_name[int32(x)]
	if ok {
		return s
	}
	return strconv.Itoa(int(x))
}
func (x Repeat) String() string {
	s, ok := Repeat_name[int32(x)]
	if ok {
		return s
	}
	return strconv.Itoa(int(x))
}
func (x ErrorType) String() string {
	s, ok := ErrorType_name[int32(x)]
	if ok {
		return s
	}
	return strconv.Itoa(int(x))
}
func (this *HyperParams) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*HyperParams)
	if !ok {
		that2, ok := that.(HyperParams)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if this.BatchSize != that1.BatchSize {
		return false
	}
	if this.NumRounds != that1.NumRounds {
		return false
	}
	if this.LearningRate != that1.LearningRate {
		return false
	}
	if this.NumLocalRounds != that1.NumLocalRounds {
		return false
	}
	if this.QuantizedUpdates != that1.QuantizedUpdates {
		return false
	}
	if this.NumClients != that1.NumClients {
		return false
	}
	return true
}
func (this *SchedulingInfo) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*SchedulingInfo)
	if !ok {
		that2, ok := that.(SchedulingInfo)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if that1.Scheduled == nil {
		if this.Scheduled != nil {
			return false
		}
	} else if !this.Scheduled.Equal(*that1.Scheduled) {
		return false
	}
	if this.Repeat != that1.Repeat {
		return false
	}
	return true
}
func (this *Work) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*Work)
	if !ok {
		that2, ok := that.(Work)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	if this.NumExamples != that1.NumExamples {
		return false
	}
	if this.NumClients != that1.NumClients {
		return false
	}
	if this.Epoch != that1.Epoch {
		return false
	}
	if !this.HyperParams.Equal(&that1.HyperParams) {
		return false
	}
	if this.TimeTaken != that1.TimeTaken {
		return false
	}
	if !this.Started.Equal(that1.Started) {
		return false
	}
	if len(this.Metrics) != len(that1.Metrics) {
		return false
	}
	for i := range this.Metrics {
		if this.Metrics[i] != that1.Metrics[i] {
			return false
		}
	}
	return true
}
func (this *ModelID) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ModelID)
	if !ok {
		that2, ok := that.(ModelID)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if this.Domain != that1.Domain {
		return false
	}
	if this.ModelType != that1.ModelType {
		return false
	}
	if this.Id != that1.Id {
		return false
	}
	return true
}
func (this *GetWorkRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*GetWorkRequest)
	if !ok {
		that2, ok := that.(GetWorkRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *FindWorkRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*FindWorkRequest)
	if !ok {
		that2, ok := that.(FindWorkRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *FindWorkResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*FindWorkResponse)
	if !ok {
		that2, ok := that.(FindWorkResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if this.AggregatorAddr != that1.AggregatorAddr {
		return false
	}
	return true
}
func (this *ModelWeightChunk) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ModelWeightChunk)
	if !ok {
		that2, ok := that.(ModelWeightChunk)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !bytes.Equal(this.Body, that1.Body) {
		return false
	}
	if this.More != that1.More {
		return false
	}
	if this.Id != that1.Id {
		return false
	}
	return true
}
func (this *GetWorkResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*GetWorkResponse)
	if !ok {
		that2, ok := that.(GetWorkResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if that1.Type == nil {
		if this.Type != nil {
			return false
		}
	} else if this.Type == nil {
		return false
	} else if !this.Type.Equal(that1.Type) {
		return false
	}
	return true
}
func (this *GetWorkResponse_Work) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*GetWorkResponse_Work)
	if !ok {
		that2, ok := that.(GetWorkResponse_Work)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Work.Equal(that1.Work) {
		return false
	}
	return true
}
func (this *GetWorkResponse_Weights) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*GetWorkResponse_Weights)
	if !ok {
		that2, ok := that.(GetWorkResponse_Weights)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Weights.Equal(that1.Weights) {
		return false
	}
	return true
}
func (this *ReportWorkRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportWorkRequest)
	if !ok {
		that2, ok := that.(ReportWorkRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if that1.Type == nil {
		if this.Type != nil {
			return false
		}
	} else if this.Type == nil {
		return false
	} else if !this.Type.Equal(that1.Type) {
		return false
	}
	return true
}
func (this *ReportWorkRequest_Work) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportWorkRequest_Work)
	if !ok {
		that2, ok := that.(ReportWorkRequest_Work)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Work.Equal(that1.Work) {
		return false
	}
	return true
}
func (this *ReportWorkRequest_Weights) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportWorkRequest_Weights)
	if !ok {
		that2, ok := that.(ReportWorkRequest_Weights)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Weights.Equal(that1.Weights) {
		return false
	}
	return true
}
func (this *ReportWorkResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportWorkResponse)
	if !ok {
		that2, ok := that.(ReportWorkResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	return true
}
func (this *ProdModelRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ProdModelRequest)
	if !ok {
		that2, ok := that.(ProdModelRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *ProdModelResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ProdModelResponse)
	if !ok {
		that2, ok := that.(ProdModelResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *ModelTypeAllocation) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ModelTypeAllocation)
	if !ok {
		that2, ok := that.(ModelTypeAllocation)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.ModelType.Equal(&that1.ModelType) {
		return false
	}
	if this.Addr != that1.Addr {
		return false
	}
	return true
}
func (this *NotifyRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*NotifyRequest)
	if !ok {
		that2, ok := that.(NotifyRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *NotifyResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*NotifyResponse)
	if !ok {
		that2, ok := that.(NotifyResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	return true
}
func (this *CancelModelTrainingRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*CancelModelTrainingRequest)
	if !ok {
		that2, ok := that.(CancelModelTrainingRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *CancelModelTrainingResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*CancelModelTrainingResponse)
	if !ok {
		that2, ok := that.(CancelModelTrainingResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	return true
}
func (this *ModelURLRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ModelURLRequest)
	if !ok {
		that2, ok := that.(ModelURLRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	return true
}
func (this *ModelURLResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ModelURLResponse)
	if !ok {
		that2, ok := that.(ModelURLResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if this.Url != that1.Url {
		return false
	}
	return true
}
func (this *Error) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*Error)
	if !ok {
		that2, ok := that.(Error)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if !this.Id.Equal(&that1.Id) {
		return false
	}
	if this.Type != that1.Type {
		return false
	}
	if this.Error != that1.Error {
		return false
	}
	if this.Stacktrace != that1.Stacktrace {
		return false
	}
	return true
}
func (this *ReportErrorRequest) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportErrorRequest)
	if !ok {
		that2, ok := that.(ReportErrorRequest)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	if len(this.Errors) != len(that1.Errors) {
		return false
	}
	for i := range this.Errors {
		if !this.Errors[i].Equal(&that1.Errors[i]) {
			return false
		}
	}
	return true
}
func (this *ReportErrorResponse) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ReportErrorResponse)
	if !ok {
		that2, ok := that.(ReportErrorResponse)
		if ok {
			that1 = &that2
		} else {
			return false
		}
	}
	if that1 == nil {
		return this == nil
	} else if this == nil {
		return false
	}
	return true
}
func (this *HyperParams) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 10)
	s = append(s, "&aggregatorpb.HyperParams{")
	s = append(s, "BatchSize: "+fmt.Sprintf("%#v", this.BatchSize)+",\n")
	s = append(s, "NumRounds: "+fmt.Sprintf("%#v", this.NumRounds)+",\n")
	s = append(s, "LearningRate: "+fmt.Sprintf("%#v", this.LearningRate)+",\n")
	s = append(s, "NumLocalRounds: "+fmt.Sprintf("%#v", this.NumLocalRounds)+",\n")
	s = append(s, "QuantizedUpdates: "+fmt.Sprintf("%#v", this.QuantizedUpdates)+",\n")
	s = append(s, "NumClients: "+fmt.Sprintf("%#v", this.NumClients)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *SchedulingInfo) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 6)
	s = append(s, "&aggregatorpb.SchedulingInfo{")
	s = append(s, "Scheduled: "+fmt.Sprintf("%#v", this.Scheduled)+",\n")
	s = append(s, "Repeat: "+fmt.Sprintf("%#v", this.Repeat)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *Work) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 12)
	s = append(s, "&aggregatorpb.Work{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "NumExamples: "+fmt.Sprintf("%#v", this.NumExamples)+",\n")
	s = append(s, "NumClients: "+fmt.Sprintf("%#v", this.NumClients)+",\n")
	s = append(s, "Epoch: "+fmt.Sprintf("%#v", this.Epoch)+",\n")
	s = append(s, "HyperParams: "+strings.Replace(this.HyperParams.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "TimeTaken: "+fmt.Sprintf("%#v", this.TimeTaken)+",\n")
	s = append(s, "Started: "+fmt.Sprintf("%#v", this.Started)+",\n")
	s = append(s, "Metrics: "+fmt.Sprintf("%#v", this.Metrics)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ModelID) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 7)
	s = append(s, "&aggregatorpb.ModelID{")
	s = append(s, "Domain: "+fmt.Sprintf("%#v", this.Domain)+",\n")
	s = append(s, "ModelType: "+fmt.Sprintf("%#v", this.ModelType)+",\n")
	s = append(s, "Id: "+fmt.Sprintf("%#v", this.Id)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *GetWorkRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.GetWorkRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *FindWorkRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.FindWorkRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *FindWorkResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.FindWorkResponse{")
	s = append(s, "AggregatorAddr: "+fmt.Sprintf("%#v", this.AggregatorAddr)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ModelWeightChunk) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 7)
	s = append(s, "&aggregatorpb.ModelWeightChunk{")
	s = append(s, "Body: "+fmt.Sprintf("%#v", this.Body)+",\n")
	s = append(s, "More: "+fmt.Sprintf("%#v", this.More)+",\n")
	s = append(s, "Id: "+fmt.Sprintf("%#v", this.Id)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *GetWorkResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 6)
	s = append(s, "&aggregatorpb.GetWorkResponse{")
	if this.Type != nil {
		s = append(s, "Type: "+fmt.Sprintf("%#v", this.Type)+",\n")
	}
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *GetWorkResponse_Work) GoString() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&aggregatorpb.GetWorkResponse_Work{` +
		`Work:` + fmt.Sprintf("%#v", this.Work) + `}`}, ", ")
	return s
}
func (this *GetWorkResponse_Weights) GoString() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&aggregatorpb.GetWorkResponse_Weights{` +
		`Weights:` + fmt.Sprintf("%#v", this.Weights) + `}`}, ", ")
	return s
}
func (this *ReportWorkRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 6)
	s = append(s, "&aggregatorpb.ReportWorkRequest{")
	if this.Type != nil {
		s = append(s, "Type: "+fmt.Sprintf("%#v", this.Type)+",\n")
	}
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ReportWorkRequest_Work) GoString() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&aggregatorpb.ReportWorkRequest_Work{` +
		`Work:` + fmt.Sprintf("%#v", this.Work) + `}`}, ", ")
	return s
}
func (this *ReportWorkRequest_Weights) GoString() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&aggregatorpb.ReportWorkRequest_Weights{` +
		`Weights:` + fmt.Sprintf("%#v", this.Weights) + `}`}, ", ")
	return s
}
func (this *ReportWorkResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 4)
	s = append(s, "&aggregatorpb.ReportWorkResponse{")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ProdModelRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.ProdModelRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ProdModelResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.ProdModelResponse{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ModelTypeAllocation) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 6)
	s = append(s, "&aggregatorpb.ModelTypeAllocation{")
	s = append(s, "ModelType: "+strings.Replace(this.ModelType.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "Addr: "+fmt.Sprintf("%#v", this.Addr)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *NotifyRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.NotifyRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *NotifyResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 4)
	s = append(s, "&aggregatorpb.NotifyResponse{")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *CancelModelTrainingRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.CancelModelTrainingRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *CancelModelTrainingResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 4)
	s = append(s, "&aggregatorpb.CancelModelTrainingResponse{")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ModelURLRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.ModelURLRequest{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ModelURLResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.ModelURLResponse{")
	s = append(s, "Url: "+fmt.Sprintf("%#v", this.Url)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *Error) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 8)
	s = append(s, "&aggregatorpb.Error{")
	s = append(s, "Id: "+strings.Replace(this.Id.GoString(), `&`, ``, 1)+",\n")
	s = append(s, "Type: "+fmt.Sprintf("%#v", this.Type)+",\n")
	s = append(s, "Error: "+fmt.Sprintf("%#v", this.Error)+",\n")
	s = append(s, "Stacktrace: "+fmt.Sprintf("%#v", this.Stacktrace)+",\n")
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ReportErrorRequest) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 5)
	s = append(s, "&aggregatorpb.ReportErrorRequest{")
	if this.Errors != nil {
		vs := make([]*Error, len(this.Errors))
		for i := range vs {
			vs[i] = &this.Errors[i]
		}
		s = append(s, "Errors: "+fmt.Sprintf("%#v", vs)+",\n")
	}
	s = append(s, "}")
	return strings.Join(s, "")
}
func (this *ReportErrorResponse) GoString() string {
	if this == nil {
		return "nil"
	}
	s := make([]string, 0, 4)
	s = append(s, "&aggregatorpb.ReportErrorResponse{")
	s = append(s, "}")
	return strings.Join(s, "")
}
func valueToGoStringAggregator(v interface{}, typ string) string {
	rv := reflect.ValueOf(v)
	if rv.IsNil() {
		return "nil"
	}
	pv := reflect.Indirect(rv).Interface()
	return fmt.Sprintf("func(v %v) *%v { return &v } ( %#v )", typ, typ, pv)
}

// Reference imports to suppress errors if they are not otherwise used.
var _ context.Context
var _ grpc.ClientConn

// This is a compile-time assertion to ensure that this generated file
// is compatible with the grpc package it is being compiled against.
const _ = grpc.SupportPackageIsVersion4

// Client API for Aggregator service

type AggregatorClient interface {
	// GetWork sends work to clients to process.
	GetWork(ctx context.Context, in *GetWorkRequest, opts ...grpc.CallOption) (Aggregator_GetWorkClient, error)
	// ReportWork is used to report the trained model/work to the server.
	ReportWork(ctx context.Context, opts ...grpc.CallOption) (Aggregator_ReportWorkClient, error)
	// Internal RPCs.
	Notify(ctx context.Context, in *NotifyRequest, opts ...grpc.CallOption) (*NotifyResponse, error)
	CancelModelTraining(ctx context.Context, in *CancelModelTrainingRequest, opts ...grpc.CallOption) (*CancelModelTrainingResponse, error)
}

type aggregatorClient struct {
	cc *grpc.ClientConn
}

func NewAggregatorClient(cc *grpc.ClientConn) AggregatorClient {
	return &aggregatorClient{cc}
}

func (c *aggregatorClient) GetWork(ctx context.Context, in *GetWorkRequest, opts ...grpc.CallOption) (Aggregator_GetWorkClient, error) {
	stream, err := grpc.NewClientStream(ctx, &_Aggregator_serviceDesc.Streams[0], c.cc, "/aggregatorpb.Aggregator/GetWork", opts...)
	if err != nil {
		return nil, err
	}
	x := &aggregatorGetWorkClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	return x, nil
}

type Aggregator_GetWorkClient interface {
	Recv() (*GetWorkResponse, error)
	grpc.ClientStream
}

type aggregatorGetWorkClient struct {
	grpc.ClientStream
}

func (x *aggregatorGetWorkClient) Recv() (*GetWorkResponse, error) {
	m := new(GetWorkResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
}

func (c *aggregatorClient) ReportWork(ctx context.Context, opts ...grpc.CallOption) (Aggregator_ReportWorkClient, error) {
	stream, err := grpc.NewClientStream(ctx, &_Aggregator_serviceDesc.Streams[1], c.cc, "/aggregatorpb.Aggregator/ReportWork", opts...)
	if err != nil {
		return nil, err
	}
	x := &aggregatorReportWorkClient{stream}
	return x, nil
}

type Aggregator_ReportWorkClient interface {
	Send(*ReportWorkRequest) error
	CloseAndRecv() (*ReportWorkResponse, error)
	grpc.ClientStream
}

type aggregatorReportWorkClient struct {
	grpc.ClientStream
}

func (x *aggregatorReportWorkClient) Send(m *ReportWorkRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *aggregatorReportWorkClient) CloseAndRecv() (*ReportWorkResponse, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(ReportWorkResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
}

func (c *aggregatorClient) Notify(ctx context.Context, in *NotifyRequest, opts ...grpc.CallOption) (*NotifyResponse, error) {
	out := new(NotifyResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Aggregator/Notify", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *aggregatorClient) CancelModelTraining(ctx context.Context, in *CancelModelTrainingRequest, opts ...grpc.CallOption) (*CancelModelTrainingResponse, error) {
	out := new(CancelModelTrainingResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Aggregator/CancelModelTraining", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for Aggregator service

type AggregatorServer interface {
	// GetWork sends work to clients to process.
	GetWork(*GetWorkRequest, Aggregator_GetWorkServer) error
	// ReportWork is used to report the trained model/work to the server.
	ReportWork(Aggregator_ReportWorkServer) error
	// Internal RPCs.
	Notify(context.Context, *NotifyRequest) (*NotifyResponse, error)
	CancelModelTraining(context.Context, *CancelModelTrainingRequest) (*CancelModelTrainingResponse, error)
}

func RegisterAggregatorServer(s *grpc.Server, srv AggregatorServer) {
	s.RegisterService(&_Aggregator_serviceDesc, srv)
}

func _Aggregator_GetWork_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetWorkRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(AggregatorServer).GetWork(m, &aggregatorGetWorkServer{stream})
}

type Aggregator_GetWorkServer interface {
	Send(*GetWorkResponse) error
	grpc.ServerStream
}

type aggregatorGetWorkServer struct {
	grpc.ServerStream
}

func (x *aggregatorGetWorkServer) Send(m *GetWorkResponse) error {
	return x.ServerStream.SendMsg(m)
}

func _Aggregator_ReportWork_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AggregatorServer).ReportWork(&aggregatorReportWorkServer{stream})
}

type Aggregator_ReportWorkServer interface {
	SendAndClose(*ReportWorkResponse) error
	Recv() (*ReportWorkRequest, error)
	grpc.ServerStream
}

type aggregatorReportWorkServer struct {
	grpc.ServerStream
}

func (x *aggregatorReportWorkServer) SendAndClose(m *ReportWorkResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *aggregatorReportWorkServer) Recv() (*ReportWorkRequest, error) {
	m := new(ReportWorkRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
}

func _Aggregator_Notify_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(NotifyRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AggregatorServer).Notify(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Aggregator/Notify",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AggregatorServer).Notify(ctx, req.(*NotifyRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _Aggregator_CancelModelTraining_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(CancelModelTrainingRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AggregatorServer).CancelModelTraining(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Aggregator/CancelModelTraining",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AggregatorServer).CancelModelTraining(ctx, req.(*CancelModelTrainingRequest))
	}
	return interceptor(ctx, in, info, handler)
}

var _Aggregator_serviceDesc = grpc.ServiceDesc{
	ServiceName: "aggregatorpb.Aggregator",
	HandlerType: (*AggregatorServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Notify",
			Handler:    _Aggregator_Notify_Handler,
		},
		{
			MethodName: "CancelModelTraining",
			Handler:    _Aggregator_CancelModelTraining_Handler,
		},
	},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "GetWork",
			Handler:       _Aggregator_GetWork_Handler,
			ServerStreams: true,
		},
		{
			StreamName:    "ReportWork",
			Handler:       _Aggregator_ReportWork_Handler,
			ClientStreams: true,
		},
	},
	Metadata: "protobuf/aggregatorpb/aggregator.proto",
}

// Client API for Edge service

type EdgeClient interface {
	// ProdModel returns the current production model.
	ProdModel(ctx context.Context, in *ProdModelRequest, opts ...grpc.CallOption) (*ProdModelResponse, error)
	// FindWork returns an address of the aggregator that the client should
	// request work from.
	FindWork(ctx context.Context, in *FindWorkRequest, opts ...grpc.CallOption) (*FindWorkResponse, error)
	// ModelURL returns a URL that can be used to download the model. For billing
	// purposes, hitting this endpoint will count as one download of the model.
	ModelURL(ctx context.Context, in *ModelURLRequest, opts ...grpc.CallOption) (*ModelURLResponse, error)
	// ReportError reports an error to the server so the developers can later view
	// them.
	ReportError(ctx context.Context, in *ReportErrorRequest, opts ...grpc.CallOption) (*ReportErrorResponse, error)
}

type edgeClient struct {
	cc *grpc.ClientConn
}

func NewEdgeClient(cc *grpc.ClientConn) EdgeClient {
	return &edgeClient{cc}
}

func (c *edgeClient) ProdModel(ctx context.Context, in *ProdModelRequest, opts ...grpc.CallOption) (*ProdModelResponse, error) {
	out := new(ProdModelResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Edge/ProdModel", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *edgeClient) FindWork(ctx context.Context, in *FindWorkRequest, opts ...grpc.CallOption) (*FindWorkResponse, error) {
	out := new(FindWorkResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Edge/FindWork", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *edgeClient) ModelURL(ctx context.Context, in *ModelURLRequest, opts ...grpc.CallOption) (*ModelURLResponse, error) {
	out := new(ModelURLResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Edge/ModelURL", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *edgeClient) ReportError(ctx context.Context, in *ReportErrorRequest, opts ...grpc.CallOption) (*ReportErrorResponse, error) {
	out := new(ReportErrorResponse)
	err := grpc.Invoke(ctx, "/aggregatorpb.Edge/ReportError", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for Edge service

type EdgeServer interface {
	// ProdModel returns the current production model.
	ProdModel(context.Context, *ProdModelRequest) (*ProdModelResponse, error)
	// FindWork returns an address of the aggregator that the client should
	// request work from.
	FindWork(context.Context, *FindWorkRequest) (*FindWorkResponse, error)
	// ModelURL returns a URL that can be used to download the model. For billing
	// purposes, hitting this endpoint will count as one download of the model.
	ModelURL(context.Context, *ModelURLRequest) (*ModelURLResponse, error)
	// ReportError reports an error to the server so the developers can later view
	// them.
	ReportError(context.Context, *ReportErrorRequest) (*ReportErrorResponse, error)
}

func RegisterEdgeServer(s *grpc.Server, srv EdgeServer) {
	s.RegisterService(&_Edge_serviceDesc, srv)
}

func _Edge_ProdModel_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ProdModelRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EdgeServer).ProdModel(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Edge/ProdModel",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EdgeServer).ProdModel(ctx, req.(*ProdModelRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _Edge_FindWork_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(FindWorkRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EdgeServer).FindWork(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Edge/FindWork",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EdgeServer).FindWork(ctx, req.(*FindWorkRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _Edge_ModelURL_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ModelURLRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EdgeServer).ModelURL(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Edge/ModelURL",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EdgeServer).ModelURL(ctx, req.(*ModelURLRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _Edge_ReportError_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ReportErrorRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EdgeServer).ReportError(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/aggregatorpb.Edge/ReportError",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EdgeServer).ReportError(ctx, req.(*ReportErrorRequest))
	}
	return interceptor(ctx, in, info, handler)
}

var _Edge_serviceDesc = grpc.ServiceDesc{
	ServiceName: "aggregatorpb.Edge",
	HandlerType: (*EdgeServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "ProdModel",
			Handler:    _Edge_ProdModel_Handler,
		},
		{
			MethodName: "FindWork",
			Handler:    _Edge_FindWork_Handler,
		},
		{
			MethodName: "ModelURL",
			Handler:    _Edge_ModelURL_Handler,
		},
		{
			MethodName: "ReportError",
			Handler:    _Edge_ReportError_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "protobuf/aggregatorpb/aggregator.proto",
}

func (m *HyperParams) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *HyperParams) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.BatchSize != 0 {
		dAtA[i] = 0x10
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.BatchSize))
	}
	if m.NumRounds != 0 {
		dAtA[i] = 0x18
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.NumRounds))
	}
	if m.LearningRate != 0 {
		dAtA[i] = 0x21
		i++
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(math.Float64bits(float64(m.LearningRate))))
		i += 8
	}
	if m.NumLocalRounds != 0 {
		dAtA[i] = 0x28
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.NumLocalRounds))
	}
	if m.QuantizedUpdates {
		dAtA[i] = 0x30
		i++
		if m.QuantizedUpdates {
			dAtA[i] = 1
		} else {
			dAtA[i] = 0
		}
		i++
	}
	if m.NumClients != 0 {
		dAtA[i] = 0x38
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.NumClients))
	}
	return i, nil
}

func (m *SchedulingInfo) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *SchedulingInfo) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.Scheduled != nil {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(types.SizeOfStdTime(*m.Scheduled)))
		n1, err := types.StdTimeMarshalTo(*m.Scheduled, dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n1
	}
	if m.Repeat != 0 {
		dAtA[i] = 0x10
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Repeat))
	}
	return i, nil
}

func (m *Work) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *Work) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n2, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n2
	if m.NumExamples != 0 {
		dAtA[i] = 0x10
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.NumExamples))
	}
	if m.NumClients != 0 {
		dAtA[i] = 0x18
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.NumClients))
	}
	if m.Epoch != 0 {
		dAtA[i] = 0x20
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Epoch))
	}
	dAtA[i] = 0x3a
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.HyperParams.Size()))
	n3, err := m.HyperParams.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n3
	if m.TimeTaken != 0 {
		dAtA[i] = 0x41
		i++
		binary.LittleEndian.PutUint64(dAtA[i:], uint64(math.Float64bits(float64(m.TimeTaken))))
		i += 8
	}
	if len(m.Metrics) > 0 {
		dAtA[i] = 0x4a
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Metrics)*8))
		for _, num := range m.Metrics {
			f4 := math.Float64bits(float64(num))
			binary.LittleEndian.PutUint64(dAtA[i:], uint64(f4))
			i += 8
		}
	}
	dAtA[i] = 0x5a
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(types.SizeOfStdTime(m.Started)))
	n5, err := types.StdTimeMarshalTo(m.Started, dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n5
	return i, nil
}

func (m *ModelID) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ModelID) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Domain) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Domain)))
		i += copy(dAtA[i:], m.Domain)
	}
	if len(m.ModelType) > 0 {
		dAtA[i] = 0x12
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.ModelType)))
		i += copy(dAtA[i:], m.ModelType)
	}
	if m.Id != 0 {
		dAtA[i] = 0x18
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Id))
	}
	return i, nil
}

func (m *GetWorkRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *GetWorkRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n6, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n6
	return i, nil
}

func (m *FindWorkRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *FindWorkRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n7, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n7
	return i, nil
}

func (m *FindWorkResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *FindWorkResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.AggregatorAddr) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.AggregatorAddr)))
		i += copy(dAtA[i:], m.AggregatorAddr)
	}
	return i, nil
}

func (m *ModelWeightChunk) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ModelWeightChunk) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Body) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Body)))
		i += copy(dAtA[i:], m.Body)
	}
	if m.More {
		dAtA[i] = 0x10
		i++
		if m.More {
			dAtA[i] = 1
		} else {
			dAtA[i] = 0
		}
		i++
	}
	if m.Id != 0 {
		dAtA[i] = 0x18
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Id))
	}
	return i, nil
}

func (m *GetWorkResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *GetWorkResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.Type != nil {
		nn8, err := m.Type.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += nn8
	}
	return i, nil
}

func (m *GetWorkResponse_Work) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.Work != nil {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Work.Size()))
		n9, err := m.Work.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n9
	}
	return i, nil
}
func (m *GetWorkResponse_Weights) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.Weights != nil {
		dAtA[i] = 0x12
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Weights.Size()))
		n10, err := m.Weights.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n10
	}
	return i, nil
}
func (m *ReportWorkRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ReportWorkRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if m.Type != nil {
		nn11, err := m.Type.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += nn11
	}
	return i, nil
}

func (m *ReportWorkRequest_Work) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.Work != nil {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Work.Size()))
		n12, err := m.Work.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n12
	}
	return i, nil
}
func (m *ReportWorkRequest_Weights) MarshalTo(dAtA []byte) (int, error) {
	i := 0
	if m.Weights != nil {
		dAtA[i] = 0x12
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Weights.Size()))
		n13, err := m.Weights.MarshalTo(dAtA[i:])
		if err != nil {
			return 0, err
		}
		i += n13
	}
	return i, nil
}
func (m *ReportWorkResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ReportWorkResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	return i, nil
}

func (m *ProdModelRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ProdModelRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n14, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n14
	return i, nil
}

func (m *ProdModelResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ProdModelResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0x1a
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n15, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n15
	return i, nil
}

func (m *ModelTypeAllocation) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ModelTypeAllocation) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.ModelType.Size()))
	n16, err := m.ModelType.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n16
	if len(m.Addr) > 0 {
		dAtA[i] = 0x12
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Addr)))
		i += copy(dAtA[i:], m.Addr)
	}
	return i, nil
}

func (m *NotifyRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *NotifyRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n17, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n17
	return i, nil
}

func (m *NotifyResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *NotifyResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	return i, nil
}

func (m *CancelModelTrainingRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *CancelModelTrainingRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n18, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n18
	return i, nil
}

func (m *CancelModelTrainingResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *CancelModelTrainingResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	return i, nil
}

func (m *ModelURLRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ModelURLRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n19, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n19
	return i, nil
}

func (m *ModelURLResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ModelURLResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Url) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Url)))
		i += copy(dAtA[i:], m.Url)
	}
	return i, nil
}

func (m *Error) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *Error) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	dAtA[i] = 0xa
	i++
	i = encodeVarintAggregator(dAtA, i, uint64(m.Id.Size()))
	n20, err := m.Id.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n20
	if m.Type != 0 {
		dAtA[i] = 0x10
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(m.Type))
	}
	if len(m.Error) > 0 {
		dAtA[i] = 0x1a
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Error)))
		i += copy(dAtA[i:], m.Error)
	}
	if len(m.Stacktrace) > 0 {
		dAtA[i] = 0x22
		i++
		i = encodeVarintAggregator(dAtA, i, uint64(len(m.Stacktrace)))
		i += copy(dAtA[i:], m.Stacktrace)
	}
	return i, nil
}

func (m *ReportErrorRequest) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ReportErrorRequest) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Errors) > 0 {
		for _, msg := range m.Errors {
			dAtA[i] = 0xa
			i++
			i = encodeVarintAggregator(dAtA, i, uint64(msg.Size()))
			n, err := msg.MarshalTo(dAtA[i:])
			if err != nil {
				return 0, err
			}
			i += n
		}
	}
	return i, nil
}

func (m *ReportErrorResponse) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *ReportErrorResponse) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	return i, nil
}

func encodeVarintAggregator(dAtA []byte, offset int, v uint64) int {
	for v >= 1<<7 {
		dAtA[offset] = uint8(v&0x7f | 0x80)
		v >>= 7
		offset++
	}
	dAtA[offset] = uint8(v)
	return offset + 1
}
func (m *HyperParams) Size() (n int) {
	var l int
	_ = l
	if m.BatchSize != 0 {
		n += 1 + sovAggregator(uint64(m.BatchSize))
	}
	if m.NumRounds != 0 {
		n += 1 + sovAggregator(uint64(m.NumRounds))
	}
	if m.LearningRate != 0 {
		n += 9
	}
	if m.NumLocalRounds != 0 {
		n += 1 + sovAggregator(uint64(m.NumLocalRounds))
	}
	if m.QuantizedUpdates {
		n += 2
	}
	if m.NumClients != 0 {
		n += 1 + sovAggregator(uint64(m.NumClients))
	}
	return n
}

func (m *SchedulingInfo) Size() (n int) {
	var l int
	_ = l
	if m.Scheduled != nil {
		l = types.SizeOfStdTime(*m.Scheduled)
		n += 1 + l + sovAggregator(uint64(l))
	}
	if m.Repeat != 0 {
		n += 1 + sovAggregator(uint64(m.Repeat))
	}
	return n
}

func (m *Work) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	if m.NumExamples != 0 {
		n += 1 + sovAggregator(uint64(m.NumExamples))
	}
	if m.NumClients != 0 {
		n += 1 + sovAggregator(uint64(m.NumClients))
	}
	if m.Epoch != 0 {
		n += 1 + sovAggregator(uint64(m.Epoch))
	}
	l = m.HyperParams.Size()
	n += 1 + l + sovAggregator(uint64(l))
	if m.TimeTaken != 0 {
		n += 9
	}
	if len(m.Metrics) > 0 {
		n += 1 + sovAggregator(uint64(len(m.Metrics)*8)) + len(m.Metrics)*8
	}
	l = types.SizeOfStdTime(m.Started)
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *ModelID) Size() (n int) {
	var l int
	_ = l
	l = len(m.Domain)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	l = len(m.ModelType)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	if m.Id != 0 {
		n += 1 + sovAggregator(uint64(m.Id))
	}
	return n
}

func (m *GetWorkRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *FindWorkRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *FindWorkResponse) Size() (n int) {
	var l int
	_ = l
	l = len(m.AggregatorAddr)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}

func (m *ModelWeightChunk) Size() (n int) {
	var l int
	_ = l
	l = len(m.Body)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	if m.More {
		n += 2
	}
	if m.Id != 0 {
		n += 1 + sovAggregator(uint64(m.Id))
	}
	return n
}

func (m *GetWorkResponse) Size() (n int) {
	var l int
	_ = l
	if m.Type != nil {
		n += m.Type.Size()
	}
	return n
}

func (m *GetWorkResponse_Work) Size() (n int) {
	var l int
	_ = l
	if m.Work != nil {
		l = m.Work.Size()
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}
func (m *GetWorkResponse_Weights) Size() (n int) {
	var l int
	_ = l
	if m.Weights != nil {
		l = m.Weights.Size()
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}
func (m *ReportWorkRequest) Size() (n int) {
	var l int
	_ = l
	if m.Type != nil {
		n += m.Type.Size()
	}
	return n
}

func (m *ReportWorkRequest_Work) Size() (n int) {
	var l int
	_ = l
	if m.Work != nil {
		l = m.Work.Size()
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}
func (m *ReportWorkRequest_Weights) Size() (n int) {
	var l int
	_ = l
	if m.Weights != nil {
		l = m.Weights.Size()
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}
func (m *ReportWorkResponse) Size() (n int) {
	var l int
	_ = l
	return n
}

func (m *ProdModelRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *ProdModelResponse) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *ModelTypeAllocation) Size() (n int) {
	var l int
	_ = l
	l = m.ModelType.Size()
	n += 1 + l + sovAggregator(uint64(l))
	l = len(m.Addr)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}

func (m *NotifyRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *NotifyResponse) Size() (n int) {
	var l int
	_ = l
	return n
}

func (m *CancelModelTrainingRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *CancelModelTrainingResponse) Size() (n int) {
	var l int
	_ = l
	return n
}

func (m *ModelURLRequest) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	return n
}

func (m *ModelURLResponse) Size() (n int) {
	var l int
	_ = l
	l = len(m.Url)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}

func (m *Error) Size() (n int) {
	var l int
	_ = l
	l = m.Id.Size()
	n += 1 + l + sovAggregator(uint64(l))
	if m.Type != 0 {
		n += 1 + sovAggregator(uint64(m.Type))
	}
	l = len(m.Error)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	l = len(m.Stacktrace)
	if l > 0 {
		n += 1 + l + sovAggregator(uint64(l))
	}
	return n
}

func (m *ReportErrorRequest) Size() (n int) {
	var l int
	_ = l
	if len(m.Errors) > 0 {
		for _, e := range m.Errors {
			l = e.Size()
			n += 1 + l + sovAggregator(uint64(l))
		}
	}
	return n
}

func (m *ReportErrorResponse) Size() (n int) {
	var l int
	_ = l
	return n
}

func sovAggregator(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
			break
		}
	}
	return n
}
func sozAggregator(x uint64) (n int) {
	return sovAggregator(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (this *HyperParams) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&HyperParams{`,
		`BatchSize:` + fmt.Sprintf("%v", this.BatchSize) + `,`,
		`NumRounds:` + fmt.Sprintf("%v", this.NumRounds) + `,`,
		`LearningRate:` + fmt.Sprintf("%v", this.LearningRate) + `,`,
		`NumLocalRounds:` + fmt.Sprintf("%v", this.NumLocalRounds) + `,`,
		`QuantizedUpdates:` + fmt.Sprintf("%v", this.QuantizedUpdates) + `,`,
		`NumClients:` + fmt.Sprintf("%v", this.NumClients) + `,`,
		`}`,
	}, "")
	return s
}
func (this *SchedulingInfo) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&SchedulingInfo{`,
		`Scheduled:` + strings.Replace(fmt.Sprintf("%v", this.Scheduled), "Timestamp", "google_protobuf1.Timestamp", 1) + `,`,
		`Repeat:` + fmt.Sprintf("%v", this.Repeat) + `,`,
		`}`,
	}, "")
	return s
}
func (this *Work) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&Work{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`NumExamples:` + fmt.Sprintf("%v", this.NumExamples) + `,`,
		`NumClients:` + fmt.Sprintf("%v", this.NumClients) + `,`,
		`Epoch:` + fmt.Sprintf("%v", this.Epoch) + `,`,
		`HyperParams:` + strings.Replace(strings.Replace(this.HyperParams.String(), "HyperParams", "HyperParams", 1), `&`, ``, 1) + `,`,
		`TimeTaken:` + fmt.Sprintf("%v", this.TimeTaken) + `,`,
		`Metrics:` + fmt.Sprintf("%v", this.Metrics) + `,`,
		`Started:` + strings.Replace(strings.Replace(this.Started.String(), "Timestamp", "google_protobuf1.Timestamp", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ModelID) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ModelID{`,
		`Domain:` + fmt.Sprintf("%v", this.Domain) + `,`,
		`ModelType:` + fmt.Sprintf("%v", this.ModelType) + `,`,
		`Id:` + fmt.Sprintf("%v", this.Id) + `,`,
		`}`,
	}, "")
	return s
}
func (this *GetWorkRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&GetWorkRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *FindWorkRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&FindWorkRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *FindWorkResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&FindWorkResponse{`,
		`AggregatorAddr:` + fmt.Sprintf("%v", this.AggregatorAddr) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ModelWeightChunk) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ModelWeightChunk{`,
		`Body:` + fmt.Sprintf("%v", this.Body) + `,`,
		`More:` + fmt.Sprintf("%v", this.More) + `,`,
		`Id:` + fmt.Sprintf("%v", this.Id) + `,`,
		`}`,
	}, "")
	return s
}
func (this *GetWorkResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&GetWorkResponse{`,
		`Type:` + fmt.Sprintf("%v", this.Type) + `,`,
		`}`,
	}, "")
	return s
}
func (this *GetWorkResponse_Work) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&GetWorkResponse_Work{`,
		`Work:` + strings.Replace(fmt.Sprintf("%v", this.Work), "Work", "Work", 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *GetWorkResponse_Weights) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&GetWorkResponse_Weights{`,
		`Weights:` + strings.Replace(fmt.Sprintf("%v", this.Weights), "ModelWeightChunk", "ModelWeightChunk", 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportWorkRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportWorkRequest{`,
		`Type:` + fmt.Sprintf("%v", this.Type) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportWorkRequest_Work) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportWorkRequest_Work{`,
		`Work:` + strings.Replace(fmt.Sprintf("%v", this.Work), "Work", "Work", 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportWorkRequest_Weights) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportWorkRequest_Weights{`,
		`Weights:` + strings.Replace(fmt.Sprintf("%v", this.Weights), "ModelWeightChunk", "ModelWeightChunk", 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportWorkResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportWorkResponse{`,
		`}`,
	}, "")
	return s
}
func (this *ProdModelRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ProdModelRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ProdModelResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ProdModelResponse{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ModelTypeAllocation) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ModelTypeAllocation{`,
		`ModelType:` + strings.Replace(strings.Replace(this.ModelType.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`Addr:` + fmt.Sprintf("%v", this.Addr) + `,`,
		`}`,
	}, "")
	return s
}
func (this *NotifyRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&NotifyRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *NotifyResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&NotifyResponse{`,
		`}`,
	}, "")
	return s
}
func (this *CancelModelTrainingRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&CancelModelTrainingRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *CancelModelTrainingResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&CancelModelTrainingResponse{`,
		`}`,
	}, "")
	return s
}
func (this *ModelURLRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ModelURLRequest{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ModelURLResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ModelURLResponse{`,
		`Url:` + fmt.Sprintf("%v", this.Url) + `,`,
		`}`,
	}, "")
	return s
}
func (this *Error) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&Error{`,
		`Id:` + strings.Replace(strings.Replace(this.Id.String(), "ModelID", "ModelID", 1), `&`, ``, 1) + `,`,
		`Type:` + fmt.Sprintf("%v", this.Type) + `,`,
		`Error:` + fmt.Sprintf("%v", this.Error) + `,`,
		`Stacktrace:` + fmt.Sprintf("%v", this.Stacktrace) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportErrorRequest) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportErrorRequest{`,
		`Errors:` + strings.Replace(strings.Replace(fmt.Sprintf("%v", this.Errors), "Error", "Error", 1), `&`, ``, 1) + `,`,
		`}`,
	}, "")
	return s
}
func (this *ReportErrorResponse) String() string {
	if this == nil {
		return "nil"
	}
	s := strings.Join([]string{`&ReportErrorResponse{`,
		`}`,
	}, "")
	return s
}
func valueToStringAggregator(v interface{}) string {
	rv := reflect.ValueOf(v)
	if rv.IsNil() {
		return "nil"
	}
	pv := reflect.Indirect(rv).Interface()
	return fmt.Sprintf("*%v", pv)
}
func (m *HyperParams) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: HyperParams: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: HyperParams: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field BatchSize", wireType)
			}
			m.BatchSize = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.BatchSize |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumRounds", wireType)
			}
			m.NumRounds = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.NumRounds |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 4:
			if wireType != 1 {
				return fmt.Errorf("proto: wrong wireType = %d for field LearningRate", wireType)
			}
			var v uint64
			if (iNdEx + 8) > l {
				return io.ErrUnexpectedEOF
			}
			v = uint64(binary.LittleEndian.Uint64(dAtA[iNdEx:]))
			iNdEx += 8
			m.LearningRate = float64(math.Float64frombits(v))
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumLocalRounds", wireType)
			}
			m.NumLocalRounds = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.NumLocalRounds |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 6:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field QuantizedUpdates", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				v |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			m.QuantizedUpdates = bool(v != 0)
		case 7:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumClients", wireType)
			}
			m.NumClients = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.NumClients |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *SchedulingInfo) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SchedulingInfo: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SchedulingInfo: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Scheduled", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if m.Scheduled == nil {
				m.Scheduled = new(time.Time)
			}
			if err := types.StdTimeUnmarshal(m.Scheduled, dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Repeat", wireType)
			}
			m.Repeat = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.Repeat |= (Repeat(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *Work) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: Work: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: Work: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumExamples", wireType)
			}
			m.NumExamples = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.NumExamples |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumClients", wireType)
			}
			m.NumClients = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.NumClients |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 4:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Epoch", wireType)
			}
			m.Epoch = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.Epoch |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field HyperParams", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.HyperParams.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 8:
			if wireType != 1 {
				return fmt.Errorf("proto: wrong wireType = %d for field TimeTaken", wireType)
			}
			var v uint64
			if (iNdEx + 8) > l {
				return io.ErrUnexpectedEOF
			}
			v = uint64(binary.LittleEndian.Uint64(dAtA[iNdEx:]))
			iNdEx += 8
			m.TimeTaken = float64(math.Float64frombits(v))
		case 9:
			if wireType == 1 {
				var v uint64
				if (iNdEx + 8) > l {
					return io.ErrUnexpectedEOF
				}
				v = uint64(binary.LittleEndian.Uint64(dAtA[iNdEx:]))
				iNdEx += 8
				v2 := float64(math.Float64frombits(v))
				m.Metrics = append(m.Metrics, v2)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
						return ErrIntOverflowAggregator
					}
					if iNdEx >= l {
						return io.ErrUnexpectedEOF
					}
					b := dAtA[iNdEx]
					iNdEx++
					packedLen |= (int(b) & 0x7F) << shift
					if b < 0x80 {
						break
					}
				}
				if packedLen < 0 {
					return ErrInvalidLengthAggregator
				}
				postIndex := iNdEx + packedLen
				if postIndex > l {
					return io.ErrUnexpectedEOF
				}
				for iNdEx < postIndex {
					var v uint64
					if (iNdEx + 8) > l {
						return io.ErrUnexpectedEOF
					}
					v = uint64(binary.LittleEndian.Uint64(dAtA[iNdEx:]))
					iNdEx += 8
					v2 := float64(math.Float64frombits(v))
					m.Metrics = append(m.Metrics, v2)
				}
			} else {
				return fmt.Errorf("proto: wrong wireType = %d for field Metrics", wireType)
			}
		case 11:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Started", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := types.StdTimeUnmarshal(&m.Started, dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ModelID) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ModelID: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ModelID: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Domain", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Domain = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ModelType", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.ModelType = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			m.Id = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.Id |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *GetWorkRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: GetWorkRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: GetWorkRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *FindWorkRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: FindWorkRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: FindWorkRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *FindWorkResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: FindWorkResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: FindWorkResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field AggregatorAddr", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.AggregatorAddr = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ModelWeightChunk) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ModelWeightChunk: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ModelWeightChunk: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Body", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				byteLen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if byteLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + byteLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Body = append(m.Body[:0], dAtA[iNdEx:postIndex]...)
			if m.Body == nil {
				m.Body = []byte{}
			}
			iNdEx = postIndex
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field More", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				v |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			m.More = bool(v != 0)
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			m.Id = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.Id |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *GetWorkResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: GetWorkResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: GetWorkResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Work", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			v := &Work{}
			if err := v.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			m.Type = &GetWorkResponse_Work{v}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Weights", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			v := &ModelWeightChunk{}
			if err := v.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			m.Type = &GetWorkResponse_Weights{v}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ReportWorkRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ReportWorkRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ReportWorkRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Work", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			v := &Work{}
			if err := v.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			m.Type = &ReportWorkRequest_Work{v}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Weights", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			v := &ModelWeightChunk{}
			if err := v.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			m.Type = &ReportWorkRequest_Weights{v}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ReportWorkResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ReportWorkResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ReportWorkResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ProdModelRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ProdModelRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ProdModelRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ProdModelResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ProdModelResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ProdModelResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ModelTypeAllocation) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ModelTypeAllocation: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ModelTypeAllocation: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ModelType", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.ModelType.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Addr", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Addr = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *NotifyRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: NotifyRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: NotifyRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *NotifyResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: NotifyResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: NotifyResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *CancelModelTrainingRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CancelModelTrainingRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CancelModelTrainingRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *CancelModelTrainingResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CancelModelTrainingResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CancelModelTrainingResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ModelURLRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ModelURLRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ModelURLRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ModelURLResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ModelURLResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ModelURLResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Url", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Url = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *Error) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: Error: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: Error: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Id", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			if err := m.Id.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Type", wireType)
			}
			m.Type = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.Type |= (ErrorType(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Error", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Error = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Stacktrace", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + intStringLen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Stacktrace = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ReportErrorRequest) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ReportErrorRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ReportErrorRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Errors", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				msglen |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if msglen < 0 {
				return ErrInvalidLengthAggregator
			}
			postIndex := iNdEx + msglen
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.Errors = append(m.Errors, Error{})
			if err := m.Errors[len(m.Errors)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func (m *ReportErrorResponse) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ReportErrorResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ReportErrorResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
			skippy, err := skipAggregator(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
				return ErrInvalidLengthAggregator
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func skipAggregator(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowAggregator
			}
			if iNdEx >= l {
				return 0, io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		wireType := int(wire & 0x7)
		switch wireType {
		case 0:
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				iNdEx++
				if dAtA[iNdEx-1] < 0x80 {
					break
				}
			}
			return iNdEx, nil
		case 1:
			iNdEx += 8
			return iNdEx, nil
		case 2:
			var length int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowAggregator
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				length |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			iNdEx += length
			if length < 0 {
				return 0, ErrInvalidLengthAggregator
			}
			return iNdEx, nil
		case 3:
			for {
				var innerWire uint64
				var start int = iNdEx
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
						return 0, ErrIntOverflowAggregator
					}
					if iNdEx >= l {
						return 0, io.ErrUnexpectedEOF
					}
					b := dAtA[iNdEx]
					iNdEx++
					innerWire |= (uint64(b) & 0x7F) << shift
					if b < 0x80 {
						break
					}
				}
				innerWireType := int(innerWire & 0x7)
				if innerWireType == 4 {
					break
				}
				next, err := skipAggregator(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
			}
			return iNdEx, nil
		case 4:
			return iNdEx, nil
		case 5:
			iNdEx += 4
			return iNdEx, nil
		default:
			return 0, fmt.Errorf("proto: illegal wireType %d", wireType)
		}
	}
	panic("unreachable")
}

var (
	ErrInvalidLengthAggregator = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowAggregator   = fmt.Errorf("proto: integer overflow")
)

func init() { proto.RegisterFile("protobuf/aggregatorpb/aggregator.proto", fileDescriptorAggregator) }

var fileDescriptorAggregator = []byte{
	// 1279 bytes of a gzipped FileDescriptorProto
	,
}
